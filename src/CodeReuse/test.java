...
public class test {
    ...
    public static void main(String[] args) {
        String str1 = new String("bing\tsefs");
    ...
    public static void getLCString(String funciton, String diff) {
        int len1, len2;
        ...
        for (i = 0; i < len2; i++) {
            for (j = len1 - 1; j >= 0; j--) {
                if (diffList[i].equals(functionList[j])) {
                    if ((i == 0) || (j == 0))
                        c[j] = 1;
                    else
        ...
        for (j = 0; j < maxLen; j++) {
            if (max[j] > 0) {
                ...
                for (i = maxIndex[j] - max[j] + 1; i <= maxIndex[j]; i++) {
                    System.out.print(functionList[i] + "\n");
                }
                System.out.println(" ");
            }
        }
