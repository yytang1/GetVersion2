...
public class CodeReuse {
	...
	public ArrayList<String> getHalfMatchFile(String diffFilePath,
		...
		for (int i = 0; i < fileList.length; i++) {
			...
			String fileName = fileList[i].replaceAll("[ |　]", " ")
					.replaceAll("\\u00A0", "").trim();
			ArrayList<String> diffList = handDiff.handleDiff(diffFilePath,
			...
			for (String functionName : functionNameList) {
				...
				for (String version : versions) {
					...
					for (String diffStr : diffList) {
						String markFunction = matchDiffLine(functionCode,
								diffStr);
						...
						if (markFunction.contains(common.markStr)
							...
							if (utils.fileExist(filePath))
								flag++;
							filePath = flag > 0 ? (path + version + "(" + flag + ").txt")
									: path + version + ".txt";
	...
	public String MostmatchDiffLine(String funciton, String diff,
			Boolean isSimilarValue, String SimilarValue) {
		// 测试
		...
		if (SimilarValue != null
				&& (similarValue - Double.parseDouble(SimilarValue) < 0)) {
			...
			for (String functionName : functionNameList) {
				...
				for (String version : versions) {
					...
					for (int j = 0; j < diffListOld.size(); j++) {
						...
						String functionFix = MostmatchDiffLine(functionCode,
	...
	public static void main(String[] args) {
		...
		ArrayList<String> diffList = handDiff.handleDiff(diffPath, fileName,
				functionName, true);
